...
func TestResolveOAuthUpstreamModel_SuffixPreservation(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			auth:  &Auth{Provider: "antigravity", Attributes: map[string]string{"auth_kind": "oauth"}},
...
func TestOAuthModelAliasChannel(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "API-key-only provider unsupported", provider: "gemini", authKind: "oauth"},
		{name: "Kimi OAuth", provider: "kimi", authKind: "oauth", want: "kimi"},
		{name: "plugin OAuth", provider: " Sample-Provider ", authKind: "oauth", want: "sample-provider"},
		{name: "plugin API key unsupported", provider: "sample-provider", authKind: "api_key"},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			if got := OAuthModelAliasChannel(tt.provider, tt.authKind); got != tt.want {
