...
func TestConvertCodexResponseToOpenAIResponses_CreatedIncludesOriginalRequestModel(t *testing.T) {
	request := []byte(`{"model":"original-codex-model","metadata":{"request_owner":"workspace-runtime"}}`)
	...
	for eventName, raw := range map[string][]byte{
		...
		if got := gjson.GetBytes(outputs[0], "response.metadata.request_owner").String(); got != "workspace-runtime" {
			t.Fatalf("%s metadata request_owner = %q, want workspace-runtime; payload=%s", eventName, got, outputs[0])
		}
	}
}
...
func TestConvertCodexResponseToOpenAIResponsesNonStreamIncomplete(t *testing.T) {
	...
	request := []byte(`{"metadata":{"request_owner":"workspace-runtime"}}`)
	out := ConvertCodexResponseToOpenAIResponsesNonStream(context.Background(), "gpt-5.5", request, nil, raw, nil)
	...
	if got := gjson.GetBytes(out, "metadata.request_owner").String(); got != "workspace-runtime" {
		t.Fatalf("metadata request_owner = %q, want workspace-runtime; payload=%s", got, out)
	}
}
